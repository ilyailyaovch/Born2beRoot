...
avram=$(free -h| awk 'NR == 2 {printf $3-0"/"$2-0"MB"}')
peravram=$(free -h| awk 'NR == 2 {printf("%.2f%%"), $3/$2*100}')
dus=$(df | awk 'NR == 4{printf ("%.2f/%.2fGb (%d%%)",$3/1024000,$2/1024000, $5)}')
Cpuload=$(top -bn1 | grep '^%Cpu' | cut -c 9- | xargs | awk '{printf("%.1f%%"), $1 + $3}')
last_reb=$(who -b | awk 'NR == 1 {print $3" " $4}')
lvmnumb=$(lsblk | grep lvm | wc -l)
mac=$(ip link show | awk '$1 == "link/ether" {print $2}')
wall"
        #Architecture: $arc
        #CPU physical: $pCpu
        #vCPU: $vCpu
        #Memory Usage: $avram ($peravram)
        #Disk Usage: $dus
        #CPU load: $Cpuload
        #Last boot: $last_reb
        #LVM use: $(if [$lvmnumb -gt 0]
        then
                echo "yes"
        else
                echo "no"
